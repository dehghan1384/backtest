...
def main():
        ...
        if st.sidebar.button("Apply preset"):
            ...
            try:
                ...
                try:
                    ...
                    st.success("Preset applied — لطفاً صفحه را رفرش یا یک ورودی را تغییر دهید تا preset اعمال شود.")
    ...
    # -------------------------
    # Download full report (single Excel with sheets + Charts image)
    # -------------------------
    ...
    if not trades_df.empty:
        ...
        # save figure to bytes (will be inserted into Excel)
        chart_img = BytesIO()
        try:
            fig.savefig(chart_img, format='png', bbox_inches='tight')
            chart_img.seek(0)
        except Exception:
            chart_img = None

        # Prefer xlsxwriter for embedding images
        try:
            import xlsxwriter
            engine = 'xlsxwriter'
        except Exception:
        ...
        try:
            if engine is not None:
                with pd.ExcelWriter(to_download, engine=engine) as writer:
                    ...
                    # equity as table (index included)
                    equity_series.to_frame(name="Equity").to_excel(writer, sheet_name="Equity", index=True)
                    summary.to_excel(writer, sheet_name="Summary", index=False)

                    # workbook & worksheet for charts
                    workbook = writer.book
                    # add a dedicated sheet for charts and insert image if available
                    chart_sheet = workbook.add_worksheet('Charts')
                    if chart_img is not None:
                        # insert image at cell B2
                        chart_sheet.insert_image('B2', 'chart.png', {'image_data': chart_img})
                    try:
                        chart_sheet.write('B1', 'Charts (Price + Indicators and Equity curve)')
                    except Exception:
                        pass
            ...
            st.download_button("Download summary CSV", csv_summary, file_name="summary.csv")
    else:
        st.info("هیچ معامله‌ای برای دانلود موجود نیست.")

    st.markdown("---")
    st.caption("Notes: Leverage is applied as a multiplier to exposure (exposure = percent_of_equity * equity * leverage). SL/TP are applied as absolute price thresholds from entry if enabled. If گزینه‌های «Enable equity-based SL/TP» فعال باشند، حدضرر/سود بر اساس درصدی از موجودی حساب به قیمت تبدیل می‌شود (مثال: 2٪ از 1000$ = 20$). Entries are only allowed inside selected time window; exits may occur due to rules or SL/TP. ")

# Launcher behaviour: if the script is executed directly (double-click or `python tester.py`),
# spawn a new process running "python -m streamlit run <this file>" and pass an env var
# BACKTEST_LAUNCHED_BY_SCRIPT=1 so the child process runs the app. The parent then exits.
if __name__ == "__main__":
    # If launched by the wrapper, env var will be set and we should run the app directly
    if os.environ.get("BACKTEST_LAUNCHED_BY_SCRIPT") == "1":
        main()
    else:
        # Launch a new process with streamlit
        env = os.environ.copy()
        env["BACKTEST_LAUNCHED_BY_SCRIPT"] = "1"
        script_path = os.path.abspath(__file__)
        cmd = [sys.executable, "-m", "streamlit", "run", script_path]

        try:
            if os.name == 'nt':
                # On Windows, open in a new console window so logs are visible
                subprocess.Popen(cmd, env=env, creationflags=subprocess.CREATE_NEW_CONSOLE)
            else:
                subprocess.Popen(cmd, env=env)
            # give the server a moment and open browser
            time.sleep(2)
            webbrowser.open("http://localhost:8501")
        except Exception as e:
            print("Failed to auto-launch Streamlit:", e)
            print("Run manually: python -m streamlit run", script_path)
